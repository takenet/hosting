            ...
            foreach (var referencedAssemblyName in referencedAssemblyNames)
            {
                ...
                catch (Exception ex)
                {
                    if (ignoreExceptionLoadingReferencedAssembly)
                        Trace.TraceError($"Could not load the referenced assembly '{referencedAssemblyName.FullName}' of assembly '{assemblyName.FullName}'");
                    else
                ...
                if (!_referencedAssembliesLoaded)
                {
                    lock (_loadAssembliesSyncRoot)
                    {
                        if (!_referencedAssembliesLoaded)
                        {
                            ...
                            {
                                Trace.TraceError("An error occurred while loading the referenced assemblies: {0}", ex);
                            }
                            finally
                            {
